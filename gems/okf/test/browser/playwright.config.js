...
import { gemRoot, bundleDir, staticPage, PORT, hostileDir, HOSTILE_PORT, v02Dir, V02_PORT, HUB_PORT,
  ...
  deeppathDir, DEEPPATH_PORT, biggraphDir, BIGGRAPH_PORT, densegraphDir, DENSEGRAPH_PORT,
  generatedOnlyDir, GENERATED_ONLY_PORT } from "./paths.js";
...
//
// OKF_NO_DISCOVERY goes with $OKF_HOME, and is not optional: cwd is the gem
// directory, which sits inside a repository that commits its own
// `.okf-registry.json`, and a project-local registry found on the path up from
// cwd *replaces* the global one. Without it these specs would rename and remove
// entries in the maintainer's committed registry — writes, into a tracked file.
const serveRegistry = (port, home) => ({
  command: `bundle exec ruby -Ilib exe/okf server -p ${port}`,
  cwd: gemRoot,
  env: { ...process.env, OKF_HOME: home, OKF_NO_DISCOVERY: "1" },
...
export default defineConfig({
  ...
  webServer: [
    ...
    serve(generatedOnlyDir, GENERATED_ONLY_PORT),
