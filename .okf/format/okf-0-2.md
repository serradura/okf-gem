...
through as a free-form producer key, and the graph page (`@okf capabilities/render`)
...
single read-time normalization on Concept (`@okf model/concept`) sufficient: both
v0.1 shapes map *into* v0.2 shapes losslessly, so everything downstream can be
taught exactly one shape.

Search (`@okf capabilities/search`) and the graph payload are **structurally
...
Concept (`@okf model/concept`) stays one class whose accessors carry §13.1's
two fallbacks inline, the validator (`@okf capabilities/validator`) warns on
the new families' shapes, and the linter (`@okf capabilities/linter`) owns the
